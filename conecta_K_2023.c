...
#include "tablero_test.h"
	...
	for (int f = 0; f < 7; f++)
	{
		for (int c = 0; c < 7; c++)
		{
			celda_inicializar(&celda_insertar);
			int color = tablero_test[f][c];
			// Si no esta vacia
			if (color != 0)
			{
				celda_poner_valor(&celda_insertar, color);
				tablero_insertar_celda(t, f, c, &celda_insertar);
			}
		}
	}
}
...
{
	// Para las 7 primeras filas
	for (int f = 0; f < 7; f++)
	{
		pantalla[f + 1][0] = 240 + f + 1;
		// Pintamos los elementos de las 7 primeras columnas
		for (int c = 0; c < 7; c++)
		{
			pantalla[0][c + 1] = 192 + c + 1;
			CELDA celda = tablero_leer_celda(t, f, c);
			if (celda_vacia(celda))
			{
				pantalla[f + 1][c + 1] = 0;
			}
			else if (celda_blanca(celda) == 0x05)
			{
				pantalla[f + 1][c + 1] = 17;
			}
			else if (celda_negra(celda) == 0x06)
			{
				pantalla[f + 1][c + 1] = 34;
			}
		}
		...
		if (tablero_fila_valida(row) && tablero_columna_valida(column) && tablero_color_valido(colour))
		{
			...
			if (celda_vacia(tablero_leer_celda(&cuadricula, row, column)))
			{
				...
				if (tablero_insertar_color(&cuadricula, row, column, colour) == EXITO)
				{
					...
					{
						while (1)
							; // equivaldria a K_linea encontrada, fin de partida...
					}
				}
				else
				{
